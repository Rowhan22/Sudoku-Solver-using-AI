...
def showErrorMessage(message):
    errorFont = pygame.font.Font('freesansbold.ttf', 30)
    errorSurf = errorFont.render(message, True, RED)
    errorRect = errorSurf.get_rect()
    errorRect.center = (WINDOWWIDTH // 2, WINDOWHEIGHT // 2)
    DISPLAYSURF.blit(errorSurf, errorRect)
    pygame.display.update()
    pygame.time.wait(1500)  # Wait for 1.5 seconds before disappearing
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == QUIT or (event.type == KEYUP and event.key == K_ESCAPE):
                ...
                # Handling number keys (1-9)
                if event.key >= K_1 and event.key <= K_9:
                    playerSolve(mousex, mousey, int(event.key) - K_0)
                # Handling Backspace to clear numbers
...
def playerSolve(x, y, number):
    if number > 0 and number <= 9:
        ...
        if boxx != None and boxy != None and SUDOKU[boxy][boxx] == 0:
            ...
            showErrorMessage("Box already filled!") 
    else:
        showErrorMessage("Invalid number! (1-9 only)")  
